...
namespace Wechat4net.Utils
{
    public class Enums
    {
        public class QY
        {
            ...
            /// <para>企业号</para>
            /// </summary>
            public enum ReceiveMessageEnum
            {
                /// <summary>
                /// 位置类型
                /// </summary>
                ...
                /// event_scancode_push
                /// </summary>
                Event_Scancode_Push = 10,
                /// <summary>
                /// 扫码推事件且弹出“消息接收中”提示框的事件
                /// event_scancode_waitmsg
                /// </summary>
                Event_Scancode_Waitmsg = 11,
                /// <summary>
                /// 弹出系统拍照发图的事件
                /// event_pic_sysphoto
                /// </summary>
                Event_Pic_Sysphoto = 12,
                /// <summary>
                /// 弹出拍照或者相册发图的事件
                /// event_pic_photo_or_album
                /// </summary>
                Event_Pic_Photo_Or_Album = 13,
                /// <summary>
                /// 弹出微信相册发图器的事件
                /// event_pic_weixin
                /// </summary>
                ...
                /// </summary>
                Event_Location_Select = 15,
                /// <summary>
                /// 用户进入应用的事件
                /// event_enter_agent
                /// </summary>
                Event_Enter_Agent = 16,
                /// <summary>
                /// 成员关注/取消关注事件
                ...
                /// <summary>
                /// 异步任务完成事件
                /// event_batch_job_result
                /// </summary>
                Event_Batch_Job_Result = 18
            }

            public static ReceiveMessageEnum getEnum(string msgType, string msgEvent)
            {
                ReceiveMessageEnum rme = ReceiveMessageEnum.Unknow;

                string check = msgType + msgEvent;
                check = check.Replace("_", "").Replace("unsubscribe", "subscribe").ToLower();

                string[] enumList = Enum.GetNames(typeof(ReceiveMessageEnum));
                for (int i = 0; i < enumList.Length; i++)
                {
                    enumList[i] = enumList[i].Replace("_", "").ToLower();
                    if (enumList[i] == check)
                    {
                        //rme = (ReceiveMessageEnum)i;
                        rme = (ReceiveMessageEnum)Enum.ToObject(typeof(ReceiveMessageEnum), i);
                        break;
                    }
        ...
        public class MP
        {
            ...
            public enum ReceiveMessageEnum
            {
                ...
                /// <summary>
                /// 扫码事件
                /// <para>用户扫描带场景值二维码时，可能推送以下两种事件：</para>
                /// <para>如果用户还未关注公众号，则用户可以关注公众号，关注后微信会将带场景值关注事件推送给开发者(关注事件)</para>
                /// <para>如果用户已经关注公众号，则微信会将带场景值扫描事件推送给开发者(扫码事件)</para>
                /// <para>event_SCAN</para>
                /// </summary>
                Event_Scan = 9,
                /// <summary>
                /// 上报地理位置事件
